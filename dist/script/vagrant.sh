...
# php
###################################################################
apt-get install -y --no-install-recommends php7.1 php7.1-fpm php7.1-cli php7.1-dev php7.1-xdebug php7.1-curl php7.1-intl php7.1-xml php7.1-zip php7.2-sqlite3
###################################################################
# composer
###################################################################
...
cp /var/www/dist/nginx/my.site.conf /etc/nginx/sites-available/
...
cd /var/www/
sudo -u www-data php -d memory_limit=-1 /usr/local/bin/composer install --prefer-dist --no-plugins